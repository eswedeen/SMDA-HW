...
SELECT * FROM actor WHERE first_name = 'Joe' OR 'Joseph';
...
SELECT last_name, COUNT(last_name) AS c FROM actor GROUP BY last_name HAVING c > 1;
...
#Copy the row from using the above command and Copy Row Unquoted
#DROP TABLE address 
...
SELECT film.film_id, film.title, COUNT(film_actor.film_id) AS 'actor_count'
FROM film
JOIN film_actor ON film_actor.film_id = film.film_id
GROUP BY film_id;

#6d. How many copies of the film Hunchback Impossible exist in the inventory system?
SELECT COUNT(film_id) AS 'Count in Inventory'
FROM inventory 
WHERE film_id IN
	(
	SELECT film_id
